...
```sh
Running "DojoReactNativePaySdkExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
...
To edit the Objective-C or Swift files, open `example/ios/DojoReactNativePaySdkExample.xcworkspace` in XCode and find the source files at `Pods > Development Pods > dojo-react-native-pay-sdk`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `dojo-react-native-pay-sdk` under `Android`.
